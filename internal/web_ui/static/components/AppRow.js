    ...
    template: `
        <template>
            ...
            <div :class="showDetails ? 'h-12' : 'h-12'"
                 class="relative flex items-center border-b border-neutral-800/50 hover:bg-neutral-900/30 transition-all group">
                ...
                <div v-for="(appSpan, spanIdx) in app.activities" :key="'app-span-'+spanIdx"
                     class="absolute h-8 top-2 rounded-sm border border-opacity-50 text-[10px] text-neutral-300 px-2 flex items-center truncate cursor-pointer hover:text-white hover:z-40 hover:shadow-lg hover:border-neutral-400 transition-all select-none bg-neutral-700 border-neutral-600"
            ...
            <template v-if="showDetails">
                <div v-for="(window, winIdx) in getWindowGroupsForApp(app)" :key="'win-group-'+winIdx"
                     class="h-12 relative flex items-center border-b border-neutral-800/50 hover:bg-neutral-900/30 transition-all group">
                    ...
                    <div v-for="(winSpan, spanIdx) in window.spans" :key="'win-span-'+spanIdx"
                         class="absolute h-8 top-2 rounded-sm border border-opacity-50 text-[10px] text-blue-300 px-2 flex items-center truncate cursor-pointer hover:text-white hover:z-40 hover:shadow-lg hover:border-blue-400 transition-all select-none bg-blue-900/30 border-blue-700"
